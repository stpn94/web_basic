	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for(Part part: request.getParts()) {
			if(!part.getName().equals("writer")){
				String contentDisposition = part.getHeader("content-disposition");// 요청한 헤더의 값을 포함하는 문자열 또는 부품에 해당 이름의 헤더가 없는 경우 null입니다.
				String uploadFileName=getUploadFileName(contentDisposition);// 파일 이름 가져오기
				part.write(uploadFileName); //스트림을 기록할 파일의 이름입니다. 파일이 MultipartConfig에 지정된 위치에 따라 생성됩니다.
